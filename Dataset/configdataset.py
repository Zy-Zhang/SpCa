...
class GLDV2Dataset_lmdb(data.Dataset):
    
    def __init__(self, db_path, seed, transforms = None, split = None, threshold_nclass = None, pool = None):
        super(GLDV2Dataset_lmdb, self).__init__()
        self.env = lmdb.open(db_path, subdir=os.path.isdir(db_path), readonly=True, lock=False, readahead=False, meminit=False)
        self.transforms = transforms
        ...
        if split is None:
            self.pool = list(range(self.length))
        else:
            self.pool = random_select(split, seed, threshold_nclass)
            self.length = len(self.pool)

    def __getitem__(self, index):
        img, label = self.read_lmdb(index)
        img = self.transforms(img)
        return img, label
    ...
    def read_lmdb(self, index):
        ...
        with env.begin(write=False) as txn:
            byteflow = txn.get(self.keys[self.pool[index]])
...
class Distractor_lmdb(data.Dataset):
    
    def __init__(self, db_path, transforms = None, imsize = None, split = None, partition = None):
        super(Distractor_lmdb, self).__init__()
        self.env = lmdb.open(db_path, subdir=os.path.isdir(db_path), readonly=True, lock=False, readahead=False, meminit=False)
        self.transforms = transforms
        self.imsize = imsize
        with self.env.begin(write=False) as txn:
        ...
        if split is None:
            self.pool = list(range(self.length))
            self.label = list(range(len(self.pool)))
        else:
            self.pool, self.label = random_select_index(split)
            self.length = len(self.pool)
        if partition is not None:
            self.pool = [self.pool[i] for i in range(partition[0], partition[1])]
            self.length = len(self.pool)
        self.pool = list(range(self.length))
        self.db_path = db_path

    def __getitem__(self, index):
        img = self.read_lmdb(index)
...
def random_select(split, seed, threshold_nclass):
    cfg = pickle.load(open('/home/574/zz1363/code/cnnimageretrieval-pytorch-1.2/cirtorch/data/train/glv2-train-clean/gnd_glv2-train-clean.pkl','rb'))
    label = torch.tensor(cfg['label'])
    pool = []
    if seed is not None:
        torch.manual_seed(seed)
    # torch.manual_seed(0)

    for idx in range(len(torch.unique(label))):
        index = torch.argwhere(label == idx).squeeze()
        num = torch.max(torch.tensor(1), torch.floor((1-split) * torch.tensor(len(index)))).type(torch.int64)
        ind = index[torch.randperm(torch.tensor(len(index)))[num:]]
        for n in ind:
            pool.append(n)
        if threshold_nclass is not None and len(pool) > threshold_nclass:
            break
    return pool

def random_select_index(split):
    cfg = pickle.load(open('/scratch/rw5/zz1363/data/google-landmark-v2/index_lmdb/gnd_glv2-index.pkl','rb'))
    label = torch.tensor(cfg['label'])
    pool = []
    label_ = []
    for idx in range(len(torch.unique(label))):
        index = torch.argwhere(label == idx).squeeze()
        if index.numel() >= split:
            for num in range(1000):
                pool.append(index[num])
                label_.append(idx)
    return pool, label_
...
class RGB_to_BGR(object):
    ...
    def __call__(self, tensor):
        tensor_ = tensor[[2,1,0],:]
        return tensor_

class RGB_to_BGR_(object):

    def __call__(self, img):
        R,G,B = img.split()
        return Image.merge("RGB", (B,G,R))

def GLDv2_lmdb(imsize, seed, split = None, convert = False, arch = 'CNN'):
    
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

    # train
    if arch == 'CLIP':
        pre = [transforms.RandomResizedCrop((224,224), scale=(0.2, 1.0), interpolation=transforms.InterpolationMode.BICUBIC),
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.2)], p=0.8),
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))]
    elif arch == 'VIT':
        pre = [transforms.RandomResizedCrop((512,512), scale=(0.2, 1.0), interpolation=transforms.InterpolationMode.BICUBIC),
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.2)], p=0.8),
            transforms.ToTensor(),
            normalize]
    else:
        pre = [transforms.RandomResizedCrop(size=imsize, scale=(0.2, 1.0)), transforms.RandomHorizontalFlip(p=0.5), 
        transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.2, 0.1)], p=0.8), transforms.ToTensor(), normalize]

    if convert:
        normalize = transforms.Normalize(mean=[0.406, 0.456, 0.485], std=[0.225, 0.224, 0.229])
        pre = [RGB_to_BGR_(),transforms.RandomResizedCrop(size=imsize, scale=(0.2, 1.0)), transforms.RandomHorizontalFlip(p=0.5), transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.2, 0.1)], p=0.8), transforms.ToTensor(), normalize]
    ...
    train_dataset = GLDV2Dataset_lmdb('/g/data/rw5/zz1363/data/google-landmark-v2/train.lmdb', seed, train_transforms, split)
    print('>> Total {} images for training'.format(len(train_dataset)))
    class_num = 81313

    return train_dataset, class_num

def GLDv2_lmdb_utils(imsize, split = None, threshold_nclass = None):
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

    # train
    pre = [transforms.RandomResizedCrop(size=imsize, scale=(0.99, 1.0)), transforms.ToTensor(), normalize]
    # pre = [transforms.Resize(int(imsize), interpolation=Image.BICUBIC), transforms.ToTensor(), normalize]
    train_transforms = transforms.Compose(pre)
    
    #create your dataset here
    train_dataset = GLDV2Dataset_lmdb('/g/data/rw5/zz1363/data/google-landmark-v2/train.lmdb', None, train_transforms, split, threshold_nclass)
    print('>> Total {} images for utils'.format(len(train_dataset)))
