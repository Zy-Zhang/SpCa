...
class GLDV2Dataset_lmdb(data.Dataset):
    ...
    def __init__(self, db_path, seed, transforms = None, pool = None):
...
def random_select(split, seed):
    ...
    for idx in range(len(torch.unique(label))):
        ...
        selected_p = torch.randperm(torch.tensor(len(index)))
        ind_t = index[selected_p[num:]]
        ind_v = index[selected_p[:num]]
...
def GLDv2_lmdb(imsize, seed, split = None, convert = False, arch = 'CNN'):
    ...
    pool_t, pool_v = random_select(split, seed)
    train_dataset = GLDV2Dataset_lmdb('/g/data/rw5/zz1363/data/google-landmark-v2/train.lmdb', seed, train_transforms, pool_t)
    val_dataset = GLDV2Dataset_lmdb('/g/data/rw5/zz1363/data/google-landmark-v2/train.lmdb', seed, val_transforms, pool_v)
    print('>> Total {} images for training and {} images for validation'.format(len(train_dataset),len(val_dataset)))
    class_num = 81313
