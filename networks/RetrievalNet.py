...
class CLIP_VPT(nn.Module):
    def __init__(self, outputdim, classifier_num, num_prompt = 3, s = 45, m = 0.2):
        super(CLIP_VPT, self).__init__()

        self.clip, _ = clipload('ViT-B/32', 'cpu')
        if num_prompt > 0:
            self.clip.apply(freeze_all_but_bn)
            self.prompt = nn.Parameter(torch.randn(num_prompt,768))
        else:
            self.prompt = None
        self.outputdim = outputdim
    ...
    def forward(self, x, label):
        ...
        if self.prompt is None:
            global_feature = self.clip.encode_image(x)
        else:
            global_feature = self.clip.encode_image(
                x, self.prompt.expand(x.shape[0], -1, -1))
        # global_feature = F.normalize(global_feature, dim = 1).squeeze()
        global_logits = self.classifier(global_feature.squeeze(), label)
        global_loss = F.cross_entropy(global_logits, label)
