...
@torch.no_grad()
def extract_vectors(net, loader, ms=[1], device=torch.device('cuda'), print_freq = 100):
    net.eval()
    vecs = torch.zeros(len(loader), net.outputdim)
    if len(ms) == 1:
        for i, input in enumerate(loader):
            if input.shape[2] < 36 or input.shape[3] < 36:
                s = max(64/input.shape[2], 64/input.shape[3])
                input = F.interpolate(input, scale_factor=s, mode='bilinear', align_corners=False)
        ...
        for i, input in enumerate(loader):
            ...
            for s in ms:
                ...
                if input_.shape[2] < 36 or input_.shape[3] < 36:
                    drop += 1
                    continue
...
def init_distributed_mode(args):
    ...
    print('| distributed init backend: {}, rank: {}, init_method: {}, world_size: {}'.format(args.dist_backend, args.rank, args.dist_url, args.world_size), flush=True)
    torch.distributed.init_process_group(backend=args.dist_backend, init_method='env://', world_size=args.world_size, rank=args.rank)
...
def freeze_weights(model, freeze=[]):
    for name, child in model.module.named_children():
        if name in freeze:
            print('The parameters of {} is freezed...'.format(name))
